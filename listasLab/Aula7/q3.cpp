...
void in_ordem_iterativa(No *pt, Pilha *topoP, int &ind){
    while(1){
        while(pt != NULL){
...
void in_ordemDecrescente_iterativa(No *pt, Pilha *topoP, int &ind){
    while(1){
        while(pt != NULL){
            empilha(topoP, pt, ind);
            if(pt->right == NULL){
...
int main(){
    ...
    cout<<endl;
    in_ordemDecrescente_iterativa(raiz, topoP, ind);
}
